	...
	if ( $product->get_type() === 'variable' ) {
		foreach ( $product->get_available_variations() as $variation ) {
			...
			if ( $variation_product->get_stock_quantity() === 0 ) {
				echo '<a href="' . esc_url( get_permalink( $variation_id ) ) . '" data-product-id="' . esc_attr( $variation_id ) . '" class="woocommerce_waitlist button hide">' . esc_html( wcwl_get_button_text( 'join' ) ) . '</a>';
			}
		}
	} else {
		if ( $product->get_stock_quantity() === 0 ) {
			echo '<a href="' . esc_url( get_permalink( $product_id ) ) . '" class="woocommerce_waitlist button">' . esc_html( wcwl_get_button_text( 'join' ) ) . '</a>';
		}
	}
	?>
