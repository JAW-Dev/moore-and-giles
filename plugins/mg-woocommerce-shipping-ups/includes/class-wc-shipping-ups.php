...
class WC_Shipping_UPS extends WC_Shipping_Method {
	...
	public function clear_transients() {
		...
		$wpdb->query( "DELETE FROM `$wpdb->options` WHERE `option_name` LIKE ('_transient_ups_saturday_quote_%') OR `option_name` LIKE ('_transient_timeout_ups_saturday_quote_%')" );
	}
	...
	public function calculate_shipping( $package = array() ) {
		...
		if ( $package_requests ) {
			...
			$saturday_rate_requests = $this->get_saturday_rate_requests( $package_requests, $package );

			if ( ! $rate_requests ) {
				$this->debug( __( 'UPS: No Services are enabled in admin panel.', 'woocommerce-shipping-ups' ) );
			}

			$rates = array_merge( $this->get_response_rates( $saturday_rate_requests, true ), $this->get_response_rates( $rate_requests ) );
	 ...
	 * @return array
	 */
	public function get_response_rates( $rate_requests, $saturday = false ) {
		...
		foreach ( $rate_requests as $code => $request ) {
			...
			$transient              = $saturday ? 'ups_saturday_quote_' . md5( $request ) : 'ups_quote_' . md5( $request );
		...
		foreach ( $ups_responses as $code => $response ) {
			...
			if ( 1 == $xml->Response->ResponseStatusCode ) {
				...
				if ( $saturday ) {
					$rates[ $rate_id . '_sat' ] = apply_filters( 'woocommerce_shipping_ups_saturday_rate', array(
						'id'    => $rate_id . '_sat',
						'label' => $rate_name . ' <small>w/ Saturday Delivery</small>',
						'cost'  => $rate_cost,
						'sort'  => $sort,
					), $currency, $xml, $this );
				} else {
	...
	/**
	 * Get XML Saturday rate requests.
	 *
	 ...
	 */
	private function get_saturday_rate_requests( $package_requests, $package ) {
		$rate_requests = array();
		...
		foreach ( $this->custom_services as $code => $params ) {
			if ( 1 == $params['enabled'] ) {
				...
				if ( $this->needs_delivery_confirmation() && 'shipment' === $this->delivery_confirmation_level( $package['destination']['country'] ) ) {
					...
					$request .= '			</DeliveryConfirmation>' . "\n";
					$request .= '           <SaturdayDelivery>1</SaturdayDelivery>' . "\n";
					$request .= '		</ShipmentServiceOptions>' . "\n";
				} else {
					$request .= '		<ShipmentServiceOptions>' . "\n";
					$request .= '           <SaturdayDelivery>1</SaturdayDelivery>' . "\n";
