...
class MG_LocationsReport extends ShoppReportFramework implements ShoppReport {
	...
	static function country ($data) {
		...
		if ( isset($countries[ $data->country ]) ) {
			return sprintf('<a href="%s">%s</a>', add_query_arg( array('report' => 'locations_state', 'country' => $data->country ) ), $countries[ $data->country ]['name']);
		}
