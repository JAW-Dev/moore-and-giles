...
class MG_ProductsReport extends ShoppReportFramework implements ShoppReport {
	...
	public static function product ( $data ) {
		$url = add_query_arg( array('s' => urlencode_deep('product:"' . trim($data->sku) . '"') ), admin_url('admin.php?page=shopp-orders') );
		return sprintf('<a href="%s">%s</a>', $url, $data->product);
	}
