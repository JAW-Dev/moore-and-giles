<?php
/*
Plugin Name: Shopp Image Data
Description:  Associate pricelines and colors with product images.
Version: 1.1
...
class ShoppImageData {
	...
	public function __construct() {
		...
		add_action( 'shopp_init', array($this,'load_color_collection') );
		add_action( 'init', array($this, 'register_colors_post_type') );

		$this->colors = get_posts( array('post_type' => 'color', 'posts_per_page' => 1000) );

		add_filter( 'shopp_themeapi_product_variationsort', array( $this, 'variation_sort' ), 10, 3 );

		// Coverimage Override
		add_filter('shopp_themeapi_product_coverimage', array($this, 'coverimage'), 20, 3);
	}

	function register_colors_post_type() {
		$labels = array(
			'name' => _x( 'Colors', 'colors' ),
			'singular_name' => _x( 'Color', 'colors' ),
			'add_new' => _x( 'Add New', 'colors' ),
			'add_new_item' => _x( 'Add New Color', 'colors' ),
			'edit_item' => _x( 'Edit Color', 'colors' ),
			'new_item' => _x( 'New Color', 'colors' ),
			'view_item' => _x( 'View Color', 'colors' ),
			'search_items' => _x( 'Search Colors', 'colors' ),
			'not_found' => _x( 'No Colors found', 'colors' ),
			'not_found_in_trash' => _x( 'No Colors found in Trash', 'colors' ),
			'parent_item_colon' => _x( 'Parent Color:', 'colors' ),
			'menu_name' => _x( 'Colors', 'colors' ),
		);
		...
		$args = array(
			...
			'query_var' => true,
			'can_export' => true,
			'rewrite' => array('with_front' => false, 'slug' => 'color'),
	...
	?>
		<table class="widefat fixed" cellspacing="0">
			...
			<?php foreach ((array)$Product->images as $i => $Image): ?>
				<?php
				$current_price = shopp_meta($Image->id, 'image', 'price');
				$colors = shopp_meta($Image->id, 'image', 'colors');
				...
				<tr>
					...
					<td class="column-price">
						<select name="shopp_image_data[<?php echo $Image->id; ?>][price]">
							<?php echo $this->get_variant_options($Product, $current_price); ?>
						</select>
					</td>
					<td class="column-color">
						<?php //print_r($this->colors); ?>
						<ul class="color-checkboxes">
							<?php foreach($this->colors as $c): ?>
								<li>
									<label><input type="checkbox" name="shopp_image_data[<?php echo $Image->id; ?>][color][]" value="<?php echo strtolower($c->post_name); ?>" <?php if(  preg_grep( "/{$c->post_name}/i" , $colors ) ) echo 'checked="checked"'; ?>  /> <?php echo $c->post_title; ?></label>
								</li>
							<?php endforeach; ?>
						</ul>
					</td>
	...
	function get_variant_options($Product, $current) {
		$result = array();
	...
	function shopp_loaded () {
		add_filter('shopp_themeapi_product_variations', array($this, 'variations'), 100, 3 );
	...
	function variations ($result, $options, $O) {
		...
		if ( Shopp::str_true($preorder) ) {
		    $add_to_cart_label = 'Pre-order Sample';
        } else {
			$add_to_cart_label = "Add to Basket";
        }
		...
		if ( 'single' == $mode ) {
			...
			foreach ($prices as $pricing) {
				...
				$image_src = ShoppStorefrontThemeAPI::image($result, array('id' => $image_id, 'width' => '180', 'quality' => '60', 'height' => '180', 'fit' => 'crop','property' => 'src'), $O);
				$src = $image_src;

				if ( $options['lazyload'] !== "false" ) {
				    $src = get_bloginfo('stylesheet_directory') . '/assets/images/grey.png';
                }
				...
				if ( 'N/A' != $pricing->type ) {
					...
					$string .= "<div class='image-variant image-variant-$pricing->id one-fourth $class'>";
						$string .= '<form action="' . shopp( 'cart.get-url' ) . '" method="post" class="shopp product validate validation-alerts">';
							$string .= '<img rel="image-variants" href="' . ShoppStorefrontThemeAPI::image($result, array('id' => $image_id, 'width' => '960', 'quality' => '60', 'height' => '960', 'fit' => 'crop','property' => 'src'), $O) . '" class="lazy colorbox-pinterest" title="' . shopp('product','get-name') . ' ' . $pricing->label . ' Leather" src="' . $src . '" data-original="' . $image_src . '" width="180" height="180" />';
							$string .=  $pricing->label;
							$string .= '<input type="hidden" name="products[' . (int)$O->id . '][prices][]" value="' . $pricing->id . '"' . $disabled . ' />';
							if ( ! $disabled ) {
								$string .= shopp( 'product.get-addtocart','ajax=html&class=addtocart single-variant-button&value=' . $add_to_cart_label );
                            }
						$string .= '</form>';
					$string .= "</div>";
...
<?php
	...
	function admin_item_image_id( $result, $Item ) {
		global $wpdb;
	...
	function load_color_collection() {
		require('color-search-collection.php');
		shopp_register_collection('ColorSearch');
	}
	...
	/**
	 * Display variation sort dropdown
	 *
	 * @param  string       $result  The output.
	 * @param  array        $options The options passed into the theme tag call.
	 * @param  ShoppProduct $product The product object.
	 * @return string       The sort dropdown output.
	 */
	function variation_sort( $result, $options, $product ) {
		$_ = array();

		$_[] = '<form action="' . get_permalink($product->id) . '" method="get" id="shopp-variation-sort" class="shopp-' . $product->slug . '-variation-sort">';
		if ( '' == get_option('permalink_structure') ) {
			foreach ($_GET as $key => $value)
				if ( 'variation_sort' != $key ) $_[] = '<input type="hidden" name="' . $key . '" value="' . $value . '" />';
		}

		$selected = 'name';
		if ( isset( $_GET['variation_sort'] ) ) $selected = $_GET['variation_sort'];

		$_[] = 'Sort By: ';
		$_[] = '<select name="variation_sort" class="shopp-variation-sort-menu">';
		$_[] = menuoptions( $this->get_variation_sort_options(), $selected, true );
		$_[] = '</select>';
		$_[] = '</form>';

		$result = join(PHP_EOL, $_);
