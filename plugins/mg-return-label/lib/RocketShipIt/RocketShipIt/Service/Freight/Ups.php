...
class Ups extends \RocketShipIt\Service\Common
{
    ...
    public function buildRequest()
    {
        ...
        $shipment->ShipmentServiceOptions = $this->buildServiceOptions();
    ...
    function buildServiceOptions()
    {
        ...
        $dopts = new \stdClass();
        if ($this->callBeforeDelivery != '') {
            $dopts->CallBeforeDeliveryIndicator = '1';
        }
        if ($this->constructionSite != '') {
            $dopts->ConstructionSiteDeliveryIndicator = '1';
        }
        if ($this->holidayDelivery != '') {
            $dopts->HolidayDeliveryIndicator = '1';
        }
        if ($this->insideDelivery != '') {
            $dopts->InsideDeliveryIndicator = '1';
        }
        if ($this->residential != '') {
            $dopts->ResidentialDeliveryIndicator = '1';
        }
        if ($this->weekendDelivery != '') {
            $dopts->WeekendDeliveryIndicator = '1';
        }
        if ($this->liftGateRequired != '') {
            $dopts->LiftGateRequiredIndicator = '1';
        }
        if ($this->saturdayDelivery != '') {
            $dopts->SaturdayDeliveryIndicator = '1';
        }
        if ($this->deliveryToDoor != '') {
            $dopts->DeliveryToDoorIndicator = '1';
        }
        if ($this->limitedAccess != '') {
            $dopts->LimitedAccessDeliveryIndicator = '1';
        }
        $opts->DeliveryOptions = $dopts;

        if ($this->sortQty != '') {
            $sort = new \stdClass();
            $sort->Quantity = $this->sortQty;
            $opts->SortingAndSegregating = $sort;
        }
    ...
    private function buildDocuments()
    {
        ...
        $image = new \stdClass();
        $type = new \stdClass();
        $type->Code = '20'; // 20 - UPS BOL, 10 - AWB, 21 - VICS BOL, 30 - Label
        $format = new \stdClass();
        ...
        $image = new \stdClass();
        $type = new \stdClass();
        $type->Code = '30';  // 30 - Label
        $format = new \stdClass();
        $format->Code = '01'; // 01 - pdf, only valid option
        $image->Type = $type;
        $image->Format = $format;
        $image->LabelsPerPage = 1; // valid values: 1, 2, 4

        $printFormat = new \stdClass();
        $printFormat->Code = '02'; // 01 - laser, 02 - Thermal
