...
class Dhl extends \RocketShipIt\Service\Common
{
    ...
    */
    ...
    public function buildDHLGlobalShipmentXml()
    {
        ...
        if ($this->labelStockType == '') {
            if ($this->labelPrintMethodCode == 'PDF') {
                $this->labelStockType = '8X4_A4_PDF';
            } else {
                $this->labelStockType = '6X4_thermal';
            }
        }
        $xml->push('Label');
            $xml->element('LabelTemplate', $this->labelStockType);
        $xml->pop(); // end Label
